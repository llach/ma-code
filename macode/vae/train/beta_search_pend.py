...
for bs in [128, 256]:
    for seed in [1, 2, 3, 4]:
        for beta in np.round(np.linspace(79, 86, 20), 2):
            ...
            with tf.Session() as s:
                v = VAE(data.shape[1:], network='pendulum', name='pendvisualuniformSEARCH',
                        beta=beta, lr=lr, latent_dim=latents, session=s)
                v.train(data, batch_size=256, num_episodes=50, print_freq=200)
